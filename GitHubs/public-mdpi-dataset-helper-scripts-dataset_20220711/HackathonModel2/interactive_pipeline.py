#!/usr/bin/env python3
"""
Interactive RF Detection Pipeline
================================
Interactive pipeline for reviewing, approving, and manually labeling RF detections.

Features:
- Review model predictions image by image
- Approve/reject model labels
- Manual labeling with drawing tools
...
class InteractiveLabelingGUI:
    """
    Interactive GUI for reviewing and correcting RF detections.
    """
    
    def __init__(self, model_path: str, packet_source: str, output_dir: str = "interactive_output"):
    ...
    def setup_gui(self):
        ...
        self.root = tk.Tk()
        self.root.title("Interactive RF Detection Pipeline")
        ...
        ttk.Button(button_frame, text="Load Spectrogram", 
        ...
        ttk.Button(button_frame, text="Next Image", 
                  command=self.load_next_image).pack(side=tk.LEFT, padx=5)
        
        # Approval buttons
        approval_frame = ttk.Frame(control_frame)
        approval_frame.grid(row=2, column=0, columnspan=3, pady=5)
        
        ttk.Button(approval_frame, text="✓ Approve All", 
                  command=self.approve_all, style="Success.TButton").pack(side=tk.LEFT, padx=5)
        ttk.Button(approval_frame, text="✗ Reject All", 
        ...
        manual_frame.grid(row=3, column=0, columnspan=3, pady=5, sticky=tk.W+tk.E)
        ...
        # Status
        self.status_var = tk.StringVar(value="Ready - Load a spectrogram to begin")
        ttk.Label(control_frame, textvariable=self.status_var).grid(row=4, column=0, columnspan=3, pady=5)
    ...
    def setup_image_display(self, parent):
        ...
        image_frame = ttk.LabelFrame(parent, text="Spectrogram", padding=5)
    ...
    def load_next_image(self):
        """Load the next image from the output directory."""
        try:
            # Look for PNG files in output directory
            output_path = Path(self.output_dir)
            if not output_path.exists():
                self.status_var.set("No output directory found. Generate spectrograms first.")
                return
                
            png_files = list(output_path.glob("*.png"))
            if not png_files:
                self.status_var.set("No PNG files found in output directory.")
                return
            ...
            if not hasattr(self, 'current_image_index'):
                ...
                self.current_image_index = (self.current_image_index + 1) % len(png_files)
            ...
            self.display_image()
            self.update_detection_list()
            self.status_var.set(f"Loaded: {image_path.name} ({self.current_image_index + 1}/{len(png_files)})")
    ...
    def load_image(self):
        ...
        file_path = filedialog.askopenfilename(
            title="Select Spectrogram Image",
    ...
    def run_detection(self):
        ...
        try:
            ...
            # Run detection using YOLO model directly
    ...
    def display_image(self):
        ...
        if len(self.current_image.shape) == 3:
            ...
            self.ax.imshow(self.current_image, cmap='viridis')
    ...
    def save_and_next(self):
        ...
        self.status_var.set("Results saved. Ready for next image.")
        self.ax.clear()
    ...
    def save_labeled_image(self):
        """Save the current image with all detection labels as PNG."""
        if self.current_image is None:
            messagebox.showwarning("Warning", "No image loaded")
            return
            
        try:
            # Create a copy of the current image for saving
            import matplotlib.pyplot as plt
            from matplotlib.patches import Rectangle
            
            # Create figure with the same size as display
            ...
            self.status_var.set(f"Labeled image saved: {os.path.basename(output_file)}")
    ...
    def save_labeled_image_silent(self):
        """Save the labeled image silently (no popup messages)."""
        if self.current_image is None:
            return
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Interactive RF Detection Pipeline")
    ...
    parser.add_argument("--output", type=str, default="interactive_output", 
                       help="Output directory")
    
    args = parser.parse_args()
    
    # Create and run GUI
    app = InteractiveLabelingGUI(args.model, args.packets, args.output)
    app.run()
